...
class CppToken(object):
    def __init__(self, value, position=None, cppdoc=None):
        self.value = value
...
class EndOfInput(CppToken):
    pass

class Keyword(CppToken):
...
class Separator(CppToken):
    VALUES = set(['(', ')', '{', '}', '[', ']', ';', ',', '.'])

class Operator(CppToken):
    MAX_LEN = 4
    VALUES = set(['>>>=', '>>=', '<<=',  '%=', '^=', '|=', '&=', '/=',
                  '*=', '-=', '+=', '<<', '--', '++', '||', '&&', '!=',
                  '>=', '<=', '==', '%', '^', '|', '&', '/', '*', '-',
...
class Annotation(CppToken):
    pass

class Identifier(CppToken):
    pass
...
class CppTokenizer(object):
    ...
    def is_cpp_identifier_start(self, c):
    ...
    def tokenize(self):
        ...
        while self.i < self.length:
            ...
            elif self.is_cpp_identifier_start(c):
            ...
            token = token_type(self.data[self.i:self.j], position, self.cppdoc)
            yield token
...
def tokenize(code, ignore_errors=False):
    tokenizer = CppTokenizer(code, ignore_errors)
