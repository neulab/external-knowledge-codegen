...
from datasets.concode.util import decanonicalize_code
...
class ConcodeEvaluator(Evaluator):
    def __init__(self, transition_system=None, args=None):
        super(ConcodeEvaluator, self).__init__()
    ...
    def evaluate_dataset(self, dataset, decode_results, fast_mode=False,
        ...
        if fast_mode:
            ...
            hypotheses = [hyp_list[0].decanonical_code_tokens
                          if hyp_list else [] for hyp_list in decode_results]

            bleu_tup = compute_bleu([[x] for x in references], hypotheses,
                                    smooth=False)
            bleu = bleu_tup[0]
            ...
            for example, hyp_list in zip(examples, decode_results):
                ...
                oracle_exact_match.append(any(hyp.is_correct
                                              for hyp in hyp_list))
